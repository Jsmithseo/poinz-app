...
const LOGGER = (0, _getLogger.default)('persistentRoomsStore');
let clientInstance;
let dbInstance;
let roomsCollection;
/**
...
async function init(config) {
  ...
  clientInstance = new _mongodb.MongoClient(config.connectionURI, {
  ...
  try {
    ...
    LOGGER.info(`connected to mongodb on   ${clientInstance.s.url}, dbName ${clientInstance.s.options.dbName}`);
    roomsCollection = dbInstance.collection(COLLECTION_NAME);
    await roomsCollection.createIndex('id', {
      unique: true,
      name: 'id_roomId'
    });
  } catch (error) {
...
async function housekeepingMarkForDeletion() {
  ...
  const updateResult = await roomsCollection.updateMany({
    ...
    // where _id is within list of found rooms
    markedForDeletion: {
      $ne: true
    } // AND not marked (flag not present or set to falsy value)
  ...
  if (roomIds.length !== updateResult.matchedCount) {
    LOGGER.warn('inconsistency during housekeepingMarkForDeletion', roomIds, updateResult.matchedCount);
  }
...
async function saveRoom(room) {
  await roomsCollection.replaceOne({
    id: room.id
  }, room, {
    upsert: true
  });
