...
const giveStoryEstimateCommandHandler = {
  ...
  preCondition: (room, command, userId) => {
    ...
    (0, _commonPreconditions.throwIfStoryIdNotFoundInRoom)(room, storyId);
...
function allValidUsersEstimated(room, storyId, userId) {
  ...
  const estimations = { ...room.stories[storyId].estimations,
...
const countAllUsersThatCanEstimate = room => Object.values(room.users || {}).filter(usr => !usr.excluded).filter(usr => !usr.disconnected).length;

var _default = giveStoryEstimateCommandHandler;
