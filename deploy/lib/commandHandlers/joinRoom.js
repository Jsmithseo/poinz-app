...
const joinRoomCommandHandler = {
  ...
  // will not check whether userId is part of the room.  for most other commands this is a precondition. not for "joinRoom".
  schema,
...
var _default = joinRoomCommandHandler;
exports.default = _default;

function joinNewRoom(room, command, userId) {
  room.applyEvent('roomCreated', {});
  const avatar = Number.isInteger(command.payload.avatar) ? command.payload.avatar : 0; // produce a "roomJoined" event for our new room
  ...
  if (command.payload.email) {
    room.applyEvent('emailSet', {
      ...
      emailHash: (0, _setEmail.calcEmailHash)(command.payload.email)
    });
  }
...
function joinExistingRoom(room, command, userId) {
  ...
  const userObject = getMatchingUserObjectFromRoom(room, command, userId); // produce a "roomJoined" event for an existing room
  // this event must contain all information about the room, such that every joining user gets the current room state!

  const joinedRoomEventPayload = {
    users: { ...room.users,
      // and all users that were already in that room
      [userObject.id]: userObject
    },
    stories: { ...room.stories
    },
    selectedStory: room.selectedStory,
    cardConfig: room.cardConfig ? room.cardConfig : _defaultCardConfig.default
  };
  room.applyEvent('joinedRoom', joinedRoomEventPayload);
  ...
  if (userObject.email) {
    room.applyEvent('emailSet', {
      ...
      emailHash: (0, _setEmail.calcEmailHash)(userObject.email)
    });
  }
