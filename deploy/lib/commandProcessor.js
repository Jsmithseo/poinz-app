...
var _sequenceQueue = _interopRequireDefault(require("./sequenceQueue"));

var _getLogger = _interopRequireDefault(require("./getLogger"));

var _commonPreconditions = require("./commandHandlers/commonPreconditions");

var _commandSchemaValidator = _interopRequireWildcard(require("./commandSchemaValidator"));
...
const LOGGER = (0, _getLogger.default)('commandProcessor');
/**
...
function commandProcessorFactory(commandHandlers, baseCommandSchema, eventHandlers, store) {
  const queue = (0, _sequenceQueue.default)(jobHandler);
  checkCommandHandlersForStructure(commandHandlers);
  const validateCmd = (0, _commandSchemaValidator.default)({ ...(0, _commandSchemaValidator.getSchemasFromRealCommandHandlers)(commandHandlers),
  ...
  async function jobHandler(job, proceed) {
    ...
    try {
      const steps = [validate, findMatchingCommandHandler, loadRoom, preConditions, handle, applyEvents, saveRoomBackToStore];
  ...
  async function loadRoom(ctx, cmd) {
    if (!cmd.roomId) {
      throw new Error('No roomId given in Command. this is invalid and should have been caught by command validation (schema).');
    } // try loading by id
  ...
  async function preConditions(ctx, cmd) {
    try {
      if (!ctx.handler.skipUserIdRoomCheck) {
        (0, _commonPreconditions.throwIfUserIdNotFoundInRoom)(ctx.room, ctx.userId);
      }
    ...
    ctx.room.applyEvent = (eventName, eventPayload) => {
      ...
      ctx.eventHandlingQueue.push(currentRoom => {
        ...
        if (updatedRoom === currentRoom) {
          throw new Error('Fatal error: Event Handlers must not return same room object! ' + eventName);
        ...
        ctx.eventsToSend.push({
          id: (0, _uuid.v4)(),
          userId: ctx.userId,
          // which user triggered the command / is "responsible" for the event
...
function checkCommandHandlersForStructure(cmdHandlers) {
  const checkSingleCmdHandlerForStructure = handlerEntry => {
    ...
    if (typeof handlerEntry[1].schema !== 'object' || handlerEntry[1].schema.constructor !== Object) {
      throw new Error(`Fatal error: "schema" on commandHandler "${handlerEntry[0]}" must be an object!`);
    }

    if (!handlerEntry[1].fn || !(handlerEntry[1].fn instanceof Function)) {
      throw new Error(`Fatal error: "fn" on commandHandler "${handlerEntry[0]}" must be a function!`);
    }
  };
...
function logCommand(command, userId) {
  if (LOGGER.isLevelEnabled('debug')) {
    LOGGER.debug(`HANDLING COMMAND user=${userId} room=${command.roomId} commandName=${command.name} commandId=${command.id}`, command);
  } else if (LOGGER.isLevelEnabled('info')) {
    LOGGER.info(`HANDLING COMMAND user=${userId} room=${command.roomId} commandName=${command.name} commandId=${command.id}`);
  }
}

function logEvents(context, correlationId) {
  if (LOGGER.isLevelEnabled('debug')) {
    LOGGER.debug(`PRODUCED EVENTS  user=${context.userId} room=${context.room.id}` + context.eventsToSend.map(e => e.name).join(', '), context.eventsToSend, `correlationId=${correlationId}`);
  } else if (LOGGER.isLevelEnabled('info')) {
    LOGGER.info(`PRODUCED EVENTS  user=${context.userId} room=${context.room.id}  ` + context.eventsToSend.map(e => e.name).join(', ') + `  correlationId=${correlationId}`);
  }
}
...
_util.default.inherits(PreconditionError, Error);